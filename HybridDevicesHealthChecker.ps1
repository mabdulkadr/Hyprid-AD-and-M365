...
try {
    ...
    Write-Host "❌ Connection to Microsoft Graph failed: $($_.Exception.Message)" -ForegroundColor Red
    exit
}
...
Write-Host "`n[1] Enter a single device name"
Write-Host "[2] Use a CSV file with a list of devices"
Write-Host "[3] Query all devices from a specific OU"
$inputChoice = Read-Host "`nSelect an input method (1-3)"

switch ($inputChoice) {
    '1' { $Device = Read-Host "`nEnter the device name (e.g., PC001)" }
    '2' { $DeviceList = Read-Host "`nEnter the full path to the device list CSV (e.g., C:\Devices.csv)" }
    '3' { $OU = Read-Host "`nEnter the OU (e.g., OU=Devices,DC=domain,DC=com or type 'all')" }
    default {
...
Write-Host "`n[1] Generate HTML report"
Write-Host "[2] Show report in console (on-screen)"
Write-Host "[3] Both HTML and On-screen"
$reportChoice = Read-Host "`nSelect a report format (1-3)"

switch ($reportChoice) {
    '1' { $HTMLReport = $true }
    '2' { $OnScreenReport = $true }
    '3' {
        $HTMLReport = $true
        $OnScreenReport = $true
    }
...
$credsChoice = Read-Host "`nUse saved credentials for MgGraph connection? (Y/N)"
if ($credsChoice -match '^[Yy]') { $SavedCreds = $true }

Write-Host "`n✅ Input summary:" -ForegroundColor Cyan
if ($Device)         { Write-Host "  → Device         : $Device" }
if ($DeviceList)     { Write-Host "  → Device List    : $DeviceList" }
if ($OU)             { Write-Host "  → OU             : $OU" }
if ($HTMLReport)     { Write-Host "  → HTML Report    : Enabled" }
if ($OnScreenReport) { Write-Host "  → On-screen Report: Enabled" }
if ($SavedCreds)     { Write-Host "  → Saved Creds    : Enabled" }
...
if (-not (Test-Path -Path $ReportFolder)) {
    try {
        ...
        Write-Host "`n📁 Created report directory: $ReportFolder" -ForegroundColor Green
    } catch {
        Write-Host "❌ Failed to create report directory: $ReportFolder. $_" -ForegroundColor Red
        exit
    }
} else {
    Write-Host "`n📁 Report directory exists: $ReportFolder" -ForegroundColor Cyan
}
...
Function CheckDevice ([String] $DeviceName){
    ...
    if (Test-WSMan -ComputerName $DeviceName -Authentication Kerberos -ErrorAction SilentlyContinue) {
        ...
        $AADDevice = Get-MgDevice -Filter "deviceId eq '$DID'" -ErrorAction SilentlyContinue
...
''
Write-host $filerep "report has been created on the path:" $ReportFolder -ForegroundColor green -BackgroundColor Black
